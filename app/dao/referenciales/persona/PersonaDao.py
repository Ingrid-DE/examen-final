...
class PersonaDao:
    ...
    def getPersonasById(self, id):
        ...
        p.id_persona, p.nombre, p.apellido, p.cedula, g.descripcion, e.descripcion, p.telefono_emergencia, c.descripcion
            FROM personas p, generos g, estado_civiles e, ciudades c
            where p.id_genero=g.id_genero and p.id_estado_civil=e.id_estado_civil and p.id_ciudad=c.id_ciudad=%s
        """
        conexion = Conexion()
        con = conexion.getConexion()
        cur = con.cursor()
    ...
    def guardarPersona(self, nombre, apellido, cedula, id_genero, id_estado_civil, telefono_emergencia, id_ciudad):
        insertPersonaSQL = """
        INSERT INTO personas(nombre, apellido, cedula, id_genero, id_estado_civil, telefono_emergencia, id_ciudad) VALUES(%s, %s, %s,%s, %s, %s, %s) RETURNING id
        """
        conexion = Conexion()
        con = conexion.getConexion()
        cur = con.cursor()
    ...
    def updatePersona(self, id, nombre, apellido, cedula, id_genero, id_estado_civil, telefono_emergencia, id_ciudad):
        updatePersonaSQL = """
        UPDATE personas
        ...
        try:
            cur.execute(updatePersonaSQL, (nombre, apellido, cedula, id, id_genero, id_estado_civil, telefono_emergencia, id_ciudad,))
